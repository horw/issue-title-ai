    ...
    def generate_content(self, prompt):
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert at improving GitHub issue titles."},
...
class DeepseekAIClient(AIClient):
    ...
    def generate_content(self, prompt):
        try:
            ...
            print(f"Error generating content with Deepseek: {str(e)}")
            raise
...
class GitHubClient:
    ...
    def get_recent_issues(self, repo, days_to_scan=7, limit=100):
        ...
        try:
            ...
            print(f"Error fetching recent issues: {str(e)}")
            raise
    ...
    def update_issue_title(self, issue, new_title):
        try:
            ...
            print(f"Error updating issue title: {str(e)}")
            raise
    ...
    def add_issue_label(self, issue, label_name):
        try:
            ...
            print(f"Error adding label to issue: {str(e)}")
            return False
    ...
    def process_issue(self, issue, github_client, auto_update=False):
        ...
        try:
            ...
            if auto_update:
                ...
                comment = (
                    f"🤖 I've improved the title of this issue for better clarity and discoverability.\n\n"
                ...
                comment = (
                    f"🤖 I've analyzed this issue title and have a suggestion for improvement:\n\n"
            ...
            print(f"Warning: Error processing issue #{issue_number}: {str(error)}")
            return {
                "issue_number": issue_number,
                "error": str(error)
            }
...
def open_issue_event(config, repo_obj, ai_client, github_client):
    ...
    try:
        ...
        print(f"Error processing issue #{config.issue_number}: {str(e)}")
        return []
